...
async function startServer() {
  ...
  // In split deployments, backend runs API-only by default.
  // Opt-in integrated frontend serving only when explicitly requested.
  const integratedFrontendDev = process.env.INTEGRATED_FRONTEND_DEV === "true";
  const serveStaticInProd = process.env.SERVE_STATIC === "true";

  if (process.env.NODE_ENV !== "production" && integratedFrontendDev) {
    const { setupVite } = await import("./vite");
    await setupVite(app, server);
  } else if (process.env.NODE_ENV === "production" && serveStaticInProd) {
